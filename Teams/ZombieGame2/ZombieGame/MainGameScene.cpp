...
void CMainGameScene::Initialize()
{
	...
	dynamic_cast<CPlayer*>(m_ObjList[OBJ_PLAYER].front())->Set_GrenadeList(&m_ObjList[OBJ_GRENADE]);
	dynamic_cast<CPlayer*>(m_ObjList[OBJ_PLAYER].front())->Set_BombList(&m_ObjList[OBJ_BOMB]);
}
...
void CMainGameScene::Late_Update()
{
	...
	if (m_ObjList[OBJ_PLAYER].front()->Get_Dead())
	{
