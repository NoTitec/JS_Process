...
void CComboMgr::Render(HDC _hdc)
{
	if (bComboTime == true)
	{
		wsprintf(sz_Combo, L"%d HIT!", iComboNumber);
		HFONT comboFont = CreateFont(30, 0, 0, 0, 0, 0, 0, 0, HANGEUL_CHARSET, 0, 0, 0, VARIABLE_PITCH | FF_ROMAN, TEXT("±Ã¼­"));
		HFONT oldComboFont = (HFONT)::SelectObject(_hdc, comboFont);
		SetBkMode(_hdc, TRANSPARENT);
		SetTextColor(_hdc, RGB(255, 49, 49));
		TextOut(_hdc, 25, WINCY / 2.0f - 200, sz_Combo, lstrlen(sz_Combo));
