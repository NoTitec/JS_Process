...
void CZombieMgr::Spawn(list<CObj*>* _pZombieList, CObj* _pTarget, int _iMaxSpawnTime, OBJ_ZOMBIE_TYPE _eZombieType)
{
	if (m_bNormalStage)
	{
		...
		bool bAllSpawn = m_iGoalZombiesNumber <= m_iTotalSpawnNumber;

		int iSelectedSpwanAreaIdx = rand() % 8;

		if (bValidSpwanTime && bValidZombieSize && bAllSpawn == false)
		{
		...
		if (m_iGoalZombiesNumber <= m_iDeathZombiesNumber)
		{
			return;
		}
	}
