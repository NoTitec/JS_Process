...
{
	m_DC = GetDC(g_hWnd);
	m_ObjList[OBJ_ID::OBJ_PLAYER].push_back(CAbstractFactory<CPlayer>::Create());
	...
	CZombieMgr::Set_Light_Zombie_Limit_Number(0);
	CZombieMgr::Set_Middle_Zombie_Limit_Number(0);
	CZombieMgr::Set_Heavy_Zombie_Limit_Number(0);
...
void CBossGameScene::Late_Update()
{
	...
	CZombieMgr::Spawn(&m_ObjList[OBJ_ZOMBIE], m_ObjList[OBJ_PLAYER].front(), 800, OBJ_LIGHT_ZOMBIE);
	CZombieMgr::Spawn(&m_ObjList[OBJ_ZOMBIE], m_ObjList[OBJ_PLAYER].front(), 500, OBJ_MIDDLE_ZOMBIE);
	CZombieMgr::Spawn(&m_ObjList[OBJ_ZOMBIE], m_ObjList[OBJ_PLAYER].front(), 200, OBJ_HEAVY_ZOMBIE);
