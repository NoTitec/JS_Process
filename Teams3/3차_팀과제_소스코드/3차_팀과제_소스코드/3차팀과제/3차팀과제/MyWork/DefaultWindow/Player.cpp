...
void CPlayer::Key_Input()
{
    ...
    if (m_bMoving)
    {
        if ((m_fCurRotRad + m_fRotRadSpeed) <= (m_fDstRotRad))
        {
            m_fCurRotRad += (m_fRotRadSpeed + m_fPreRotRad);
...
void CPlayer::Input_Mouse()
{
    if (KEYMGR->Key_Down(VK_LBUTTON))
    {
        ...
        // Look, Right RotZ
        {
            D3DXMatrixRotationZ(&m_matRotZ, m_fPreRotRad);
