...
{
	// 클라이언트 생성 시점이 아닌 VIEW 객체 생성 시점에 호출

	// CreateStatic : 분할 창을 만드는 함수
	// CreateStatic(부모 윈도우 주소, 행의 수, 열의 수, 창 스타일, 자식창 ID 값)

	//m_MainSplitter.CreateStatic(this, 1, 2);
	//
	//// CreateView : 분할된 창에 표시할 View 객체(창)를 만드는 함수
	//// CreateView(배치할 행, 배치할 열, view 창을 새로 생성, 초기 크기, pContext)
	//
	...
	// IdFromRowCol(행, 열) : 지정한 행과 열에 해당하는 창의 id값 반환
